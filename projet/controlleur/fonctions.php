...
//TROUVER L'EMAIL D'UN UTILISATEUR EN SACHANT SON PSEUDO
function chercher_email_utilisateur($pseudo)
{
	addslashes($pseudo);
	$connexion = mysqli_connect(HOST, USER, PASSWORD, DATABASE);
	...
	if(mysqli_error($connexion))
		die("Erreur Connection");
	$req = "SELECT email FROM utilisateur WHERE pseudo='$pseudo'";
	$i=0;
	if($resultat = mysqli_query($connexion, $req))
		while($ligne = mysqli_fetch_assoc($resultat))
			$i = $ligne['email'];
...
//TROUVER LE TELEPHONE D'UN UTILISATEUR EN SACHANT SON PSEUDO
function chercher_telephone_utilisateur($pseudo)
{
	addslashes($pseudo);
	$connexion = mysqli_connect(HOST, USER, PASSWORD, DATABASE);
	...
	if(mysqli_error($connexion))
		die("Erreur Connection");
	$req = "SELECT telephone FROM utilisateur WHERE pseudo='$pseudo'";
	$i=0;
	if($resultat = mysqli_query($connexion, $req))
		while($ligne = mysqli_fetch_assoc($resultat))
			$i = $ligne['telephone'];
...
//TROUVER LE TYPE D'UN UTILISATEUR EN SACHANT SON PSEUDO
function chercher_type_utilisateur($pseudo)
{
	addslashes($pseudo);
	$connexion = mysqli_connect(HOST, USER, PASSWORD, DATABASE);
	...
	if(mysqli_error($connexion))
		die("Erreur Connection");
	$req = "SELECT type FROM utilisateur WHERE pseudo='$pseudo'";
	$i=0;
	if($resultat = mysqli_query($connexion, $req))
		while($ligne = mysqli_fetch_assoc($resultat))
			$i = $ligne['type'];
...
//ELLE PERMET DE TROUVER L'ID D'UN UTILISATEUR EN SCAHENT SON PSEUDO ELLE RETOURNE CET ID
function chercher_id_utilisateur($pseudo)
{
 	addslashes($pseudo);
	$connexion = mysqli_connect(HOST, USER, PASSWORD, DATABASE);
	...
	if(mysqli_error($connexion))
		die("Erreur Connection");
	$req = "SELECT id FROM utilisateur WHERE pseudo='$pseudo'";
	$i=0;
	if($resultat = mysqli_query($connexion, $req))
		while($ligne = mysqli_fetch_assoc($resultat))
			$i = $ligne['id'];
...
function ajouter_une_publication($pseudo, $contenu, $categorie)
{
	...
	$id_utilisateur = chercher_id_utilisateur($pseudo);
